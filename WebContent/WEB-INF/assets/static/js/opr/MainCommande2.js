$(function() {
	$.BaseCRUD({
		listURL : "mainCommande?goto=listMDC2",
	...
	$("#btnAdd2").click(
			function() {
				$.post("franchises?goto=clickAssets&as=4", function(data) {
					if (data.suc == 1) {
						$('#dlg').dialog('center').dialog('open').dialog(
								'setTitle', "新增订单信息");
						$('#fm').form('clear');
						if ($("#validator_id")) {
							$("#validator_id").val(0);
						...
						$.messager.alert('提示', '未分配新增订单权限', 'warning');
					}
				});
			...
			function() {
				...
				if (r) {
					...
					$.messager.alert($.i18n.prop('crud.dialog.tip.title'),
							$.i18n.prop('crud.dialog.tip.update'), 'warning');
				}
			});
	...
	$("#btnSave2").click(
			function() {
				...
				if (vid == 0) {
					...
					if ($("#fm").form('validate')) {
						$.post("mainCommande?goto=update",
								$("#fm").serialize(), function(data) {
									$('#dlg').dialog('close');
									if (data.suc == 1) {
										$('#tt').datagrid('reload');
										msg = "<span class='tip_suc'>"
...
var typeSelect = function(r) {
	...
	$("#c5").combobox({
		onLoadSuccess : function() {
			if (row != null && row.seller != null) {
				$("#c5").combobox('select', row.seller.id);
			} else if (row != null && row.fid != null) {
				$("#c5").combobox('select', row.fid.id);
			}
		}
	});
}
...
function formatterStatus(status) {
	switch (status) {
	case 1:
		return $.i18n.prop('mCommande.state.s1');
	case 2:
		return $.i18n.prop('mCommande.state.s2');
	case 3:
		return $.i18n.prop('mCommande.state.s3');
	case 4:
		return $.i18n.prop('mCommande.state.s4');
	case 5:
		return $.i18n.prop('mCommande.state.s5');
	case 6:
		return $.i18n.prop('mCommande.state.s6');
	case 7:
		return $.i18n.prop('mCommande.state.s7');
	case 8:
		return $.i18n.prop('mCommande.state.s8');
	case 9:
		return $.i18n.prop('mCommande.state.s9');
	case 10:
		return $.i18n.prop('mCommande.state.s10');
	case 11:
		return $.i18n.prop('mCommande.state.s11');
	case 12:
		return $.i18n.prop('mCommande.state.s12');
	default:
		return "";
	}
}