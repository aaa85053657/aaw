(function($) {
	...
	// filter types
	$.fn.datagrid.defaults.filters = $.extend({},
	...
	$.fn.datagrid.defaults.operators = {
		...
		endwith : {
			text : 'End With',
			isMatch : function(source, value) {
				return source.toLowerCase().indexOf(value.toLowerCase(),
						source.length - value.length) !== -1;
			}
		},
		less : {
			text : 'Less',
	...
	function resizeFilter(target, field, width) {
		...
		ff.each(function() {
			...
			var btn = $(this).closest('div.datagrid-filter-c').find(
					'a.datagrid-filter-btn');
			if (width != undefined) {
	...
	function getFilterComponent(target, field) {
		...
		return header.find('tr.datagrid-filter-row td[field="' + field
				+ '"] input.datagrid-filter');
	}
	...
	function addFilterRule(target, param) {
		...
		if (input.length) {
			...
			if (menu) {
				menu.find('.' + opts.filterMenuIconCls).removeClass(
						opts.filterMenuIconCls);
				var item = menu.menu('findItem',
						opts.operators[param.op]['text']);
				menu.menu('setIcon', {
					target : item.target,
	...
	function removeFilterRule(target, field) {
		...
		if (field) {
			...
			opts.filterRules = [];
			var fields = dg.datagrid('getColumnFields', true).concat(
					dg.datagrid('getColumnFields'));
			_clear(fields);
		}
	...
	function init(target, filters) {
		...
		function createFilter(frozen) {
			...
			var table = (frozen ? dc.header1 : dc.header2)
					.find('table.datagrid-htable');
			...
			for (var i = 0; i < fields.length; i++) {
				...
				var div = $('<div class="datagrid-filter-c"></div>').appendTo(
						td);
		...
		function createFilterButton(container, operators) {
			...
			var btn = $('<a class="datagrid-filter-btn">&nbsp;</a>').addClass(
					opts.filterBtnIconCls);
