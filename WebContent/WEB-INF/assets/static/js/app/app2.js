var timer_json = {
	"INVAILD_LOGIN" : 3,
	"CONTINUE_SCAN" : 10,
	"SELECT_TIME" : 10,
	"PROCESS_DONE" : 3,
	"ORDER_BEGIN" : 5,
	"LOCK_TIME" : 120
};

function deal(len, idx) {
	var ck = $("#ckrb_" + idx).is(":checked");
	for (var i = 0; i < len; i++) {
		$("#ckrb_" + i).prop("checked", false);
	}
	// $("#ckrb_" + idx).prop("checked", true);
	// $("#ckrb_" + idx).prop("checked", true);
	for (var i = idx; i < len; i++) {
		$("#ckrb_" + i).prop("checked", true);
	}
}

$(function() {
	document.getElementById("firstInput").focus();

	$.get("static/app_timeout_cfg.json", function(rs) {
		if (rs) {
			timer_json = rs;
		}
	}, "json");

	var index = layer
			.open({
				type : 1,
				content : $('#test7'),
				area : 'auto',
				title : '激活态->订单开始（<span style="color:red;" id="order_start"></span>秒倒计时）'
			});
	dyncTimer(timer_json["ORDER_BEGIN"], function() {
		closeLayer(index);
	}, function(sec) {
		$("#order_start").text(sec);
	});
});
function openExcepiton() {
	var wid = $("#worksheetIsWorking").val();
	$
			.post(
					'ProductionException?act=combobox',
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							var html = "";
							$
									.each(
											data,
											function(index, item) {
												var name = item.text;
												var id = item.id;
												html += "<div><input name='er' type='checkbox' value='"
														+ id
														+ "'><label>"
														+ name
														+ "</label></div>";
											});
							$("#ex_resonDiv").html(html);
						}
					});
	$
			.post(
					'worksheetdefatil?goto=getWorksheet&wid=' + wid,
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							$("#worksheetDetailId").html("");
							$("#rollBackId").html("");
							var len = data.length;
							$
									.each(
											data,
											function(index, item) {
												var name = data[index].name;
												var id = data[index].id;
												$("#worksheetDetailId")
														.html(
																$(
																		"#worksheetDetailId")
																		.html()
																		+ "<input type='checkbox' name='worksheetDetailId' value="
																		+ id
																		+ " checked='checked'/>"
																		+ name);
												$("#rollBackId")
														.html(
																$("#rollBackId")
																		.html()
																		+ "<input type='checkbox' name='rollBackId' value='"
																		+ id
																		+ "' checked='checked' onchange='deal("
																		+ len
																		+ ","
																		+ index
																		+ ")' id='ckrb_"
																		+ index
																		+ "' />"
																		+ name);

											});
							layer
									.open({
										type : 1,
										content : $('#test'),
										area : [ '600px', '400px' ],
										title : '异常处理',
										btn : [ '确定', '取消' ],
										yes : function(index, layero) {
											var index = layer
													.confirm(
															'确认提交？',
															{
																btn : [ '是',
																		'否' ]
															},
															function() {
																var flag = $(
																		"input[name='er']")
																		.is(
																				':checked');
																if (flag) {
																	$(
																			'#exceptionfm')
																			.form(
																					'submit',
																					{
																						url : 'worksheetdefatil?goto=saveExcption&wid2='
																								+ wid,
																						onSubmit : function() {
																							return $(
																									this)
																									.form(
																											'validate');
																						},
																						success : function(
																								r) {
																							layer
																									.open({
																										content : '异常提交成功，操作流程已回滚！',
																										btn : '我知道了',
																										yes : function(
																												index,
																												layero) {
																											window.location.href = "app";
																										}
																									});
																						}
																					})
																} else {
																	alert("请选择异常原因！");
																}
															},
															function() {
																closeLayer(index);
															});
										},
										cancel : function(index) {
											closeLayer(index);
										}
									});
						}
					});
}
function openOthers() {
	$
			.post(
					"worksheet?goto=findWorking",
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							var tableHTML = "<table class='gridtable'>";
							tableHTML += "<tr><th class='col1'>订单号</th><th  class='col2'>副单号</th><th  class='col2'>第三方ID</th><th  class='col2'>型号</th><th  class='col2'>工艺师</th><th>操作</th></tr>";
							if (data != null) {
								var clickId = $("#worksheetId").val();
								$
										.each(
												data,
												function(index, e) {
													var code = e.slaveCommande.mainCommande.codeId;
													var type = e.slaveCommande.model.name;
													var opr = e.opr.name;
													var externalCode = e.slaveCommande.mainCommande.externalCode;
													var code2 = e.slaveCommande.codeId;
													if (clickId != e.id) {
														tableHTML += "<tr><td  class='col1'>"
																+ code
																+ "</td><td class='col2'>"
																+ code2
																+ "</td><td class='col2'>"
																+ externalCode
																+ "</td><td class='col2'>"
																+ type
																+ "</td><td class='col2'>"
																+ opr
																+ "</td><td><a href='javascript:lookMain("
																+ e.id
																+ ")'>查看</a></td></tr>";
													} else {
														tableHTML += "<tr><td  class='col1'>"
																+ code
																+ "</td><td class='col2'>"
																+ code2
																+ "</td><td class='col2'>"
																+ externalCode
																+ "</td><td class='col2'>"
																+ type
																+ "</td><td class='col2'>"
																+ opr
																+ "</td><td></td></tr>";
													}
												});
							}
							tableHTML += "</table>";
							$(".other_order").html(tableHTML);
							layer.open({
								type : 1,
								content : $('#test2'),
								area : [ '600px', '300px' ],
								title : '<span>其他订单</span>'
							});
						}
					});
}

function openCell() {
	layer.open({
		type : 1,
		content : $('#test3'),
		area : 'auto',
		title : '呼叫'
	});
}

function chang(val) {
	var flag = 1;
	var id = val.value;
	var code = $("#wcodeMain").val();
	var ename = $("#ename").val();
	var dmname = $("#dmname").val();
	if (code == id) {
		var msg = "订单号:" + code + "由 工艺师:" + ename + "完成 工序:" + dmname;
		layer
				.confirm(
						msg,
						{
							btn : [ '是', '否' ]
						// 按钮
						},
						function() {
							var url = "worksheetdefatil?goto=working";
							if ($('#fm1').form('validate')) {
								$
										.post(
												url,
												$("#fm1").serialize(),
												function() {
													var index = layer
															.open({
																title : '工序结束（<span style="color:red;" id="over_1"></span>秒倒计时）',
																content : "完成订单",
																btn : '我知道了',
																yes : function(
																		index,
																		layero) {
																	$
																			.post(
																					"worksheet?goto=findWorking",
																					function(
																							data) {
																						if (data != null
																								&& "" != data) {
																							openOthers()
																							flag = 2;
																						} else {
																							window.location.href = "app";
																							flag = 2;
																						}
																					});
																},
																end : function() {
																	window.location.href = "app";
																}
															});
													dyncTimer(
															timer_json["PROCESS_DONE"],
															function() {
																if (flag == 1) {
																	$
																			.post(
																					"worksheet?goto=findWorking",
																					function(
																							data) {
																						if (data != null
																								&& "" != data) {
																							openOthers()
																						} else {
																							window.location.href = "app";
																						}
																					});
																}
															},
															function(sec) {
																$("#over_1")
																		.text(
																				sec);
															});
												});
							}
						})
	} else {
		$
				.post(
						'worksheet?goto=findByCode',
						{
							id : id
						},
						function(data) {
							if (typeof (data) == 'string') {
								var index = layer
										.open({
											title : '用户超时'
													+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
											content : '登录用户已超时，请重新登录！',
											btn : '我知道了',
											yes : function(index, layero) {
												window.location.href = "login";
											}
										});
								dyncTimer(timer_json["CONTINUE_SCAN"],
										function() {
											closeLayer(index);
											window.location.href = "login";
										}, function(sec) {
											$("#overTime_1").text(sec);
										});
							} else {
								if (data.type == 0) {
									var msg = data.msg;
									var index = layer
											.open({
												type : 0,
												content : msg,
												area : 'auto',
												title : '无效登录（<span style="color:red;" id="no_login"></span>秒倒计时）'
											});
									dyncTimer(timer_json["INVAILD_LOGIN"],
											function() {
												closeLayer(index);
											}, function(sec) {
												$("#no_login").text(sec);
												$("#firstInput").val('');
												document.getElementById(
														"firstInput").focus();
											});

								} else if (data.type == 5) {
									var index = layer
											.open({
												type : 0,
												content : '该加工单已被终止或删除！',
												area : 'auto',
												title : '无效登录（<span style="color:red;" id="no_lg2"></span>秒倒计时）'
											});
									dyncTimer(timer_json["INVAILD_LOGIN"],
											function() {
												closeLayer(index);
											}, function(sec) {
												$("#no_login").text(sec);
												$("#firstInput").val('');
												document.getElementById(
														"firstInput").focus();
											});
								} else if (data.type == 1) {
									$('#wcode')
											.html(
													data.object.slaveCommande.mainCommande.codeId
															+ ","
															+ data.object.slaveCommande.codeId);
									$('#wid').val(data.object.id);
									$("#secondInput1").val('');
									var index = layer
											.open({
												type : 1,
												content : $('#test5'),
												area : '400px',
												title : '中间态->继续扫描(<span style="color:red;" id="mid_status1"></span>秒倒计时)'
											});
									dyncTimer(
											timer_json["CONTINUE_SCAN"],
											function() {
												closeLayer(index);
											},
											function(sec) {
												$("#mid_status1").text(sec);
												$("#firstInput").val('');
												document.getElementById(
														"secondInput1").focus();
											});
								} else if (data.type == 2) {
									$('#ecode').html(data.object.name);
									$('#eid').val(data.object.id);
									$("#secondInput2").val('');
									var index = layer
											.open({
												type : 1,
												content : $('#test6'),
												area : '400px',
												title : '中间态->继续扫描(<span style="color:red;" id="mid_status2"></span>秒倒计时)'
											});
									dyncTimer(
											timer_json["CONTINUE_SCAN"],
											function() {
												closeLayer(index);
											},
											function(sec) {
												$("#mid_status2").text(sec);
												$("#firstInput").val('');
												document.getElementById(
														"secondInput2").focus();
											});
								} else if (data.type == 3) {
									var index = layer
											.open({
												type : 0,
												content : '其他工作台正在操作此订单!',
												area : 'auto',
												title : '无效登录（<span style="color:red;" id="ngl_1"></span>秒倒计时）'
											});
									dyncTimer(timer_json["INVAILD_LOGIN"],
											function() {
												closeLayer(index);
											}, function(sec) {
												$("#ngl_1").text(sec);
												$("#firstInput").val('');
												document.getElementById(
														"firstInput").focus();
											});
								}
							}
						})
	}
}

function validEcode(value) {
	var id = $('#wid').val();
	var code = value.value;
	$
			.post(
					'worksheet?goto=findByCode2',
					{
						id : id,
						code : code,
						type : 1
					},
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							if (data.type == 1) {
								var url = "worksheetdefatil?goto=workingTemp";
								$.post(url, $("#fm1").serialize(), function() {
									$("#eidCom").val('');
									$("#didCom").val('');
									var did = data.detail.id;
									var eid = data.employee.id;
									$("#eidCom").val(eid);
									$("#didCom").val(did);
									$("#ot").val(timer_json["LOCK_TIME"]);
									$("#workForm").attr('action',
											'worksheet?goto=appView');
									$("#workForm").submit();

								});
							} else if (data.type == 2) {
								var index = layer
										.confirm(
												'操作人员不对，是否需要更换操作人员？',
												{
													btn : [ '替换,并返回订单',
															'不替换,并返回订单', '取消' ],
													btn3 : function(i, l) {
														closeLayer(index);
														$("#firstInput")
																.val('');
														document
																.getElementById(
																		"firstInput")
																.focus();
													}
												},
												function() {
													var did = data.detail.id;
													var eid = data.employee.id;
													var url = "worksheetdefatil?goto=workingTemp";
													$
															.post(
																	url,
																	$("#fm1")
																			.serialize(),
																	function() {
																		$(
																				"#eidCom")
																				.val(
																						'');
																		$(
																				"#didCom")
																				.val(
																						'');
																		var did = data.detail.id;
																		var eid = data.employee.id;
																		$(
																				"#eidCom")
																				.val(
																						eid);
																		$(
																				"#didCom")
																				.val(
																						did);
																		$("#ot")
																				.val(
																						'');
																		$(
																				"#workForm")
																				.attr(
																						'action',
																						'worksheet?goto=appView');
																		$
																				.post(
																						'worksheet?goto=updateEMP',
																						{
																							did : did,
																							eid : eid
																						},
																						function(
																								data) {
																							$(
																									"#workForm")
																									.submit();
																						})
																	});
												},
												function() {
													$("#eidCom").val('');
													$("#didCom").val('');
													var did = data.detail.id;
													$("#ot").val('');
													$("#didCom").val(did);
													$("#workForm")
															.attr('action',
																	'worksheet?goto=appView3');
													$("#workForm").submit();

												});
							} else if (data.type == 3) {
								var index = layer
										.open({
											type : 0,
											content : '其他工作台正在操作此订单!',
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ngl_2"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ngl_2").text(sec);
											$("#firstInput").val('');
											document.getElementById(
													"firstInput").focus();
										});
							} else if (data.type == 5) {
								var index = layer
										.open({
											type : 0,
											content : '改加工单已被终止或删除！',
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ngl4"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ngl_3").text(sec);
											$("#firstInput").val('');
											document.getElementById(
													"firstInput").focus();
										});
							} else {
								var index = layer
										.open({
											type : 0,
											content : '无法识别代码,请先扫描订单条码或者工牌,并按后续提示操作',
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ngl_3"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ngl_3").text(sec);
											$("#firstInput").val('');
											document.getElementById(
													"firstInput").focus();
										});
							}
						}
					});
}

function validWcode(value) {
	var id = $('#eid').val();
	var code = value.value;
	$
			.post(
					'worksheet?goto=findByCode2',
					{
						id : id,
						code : code,
						type : 2
					},
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							if (data.type == 1) {
								$("#eidCom").val('');
								$("#didCom").val('');
								var did = data.detail.id;
								var eid = data.employee.id;
								$("#ot").val(timer_json["LOCK_TIME"]);
								$("#eidCom").val(eid);
								$("#didCom").val(did);
								$("#workForm").attr('action',
										'worksheet?goto=appView');
								$("#workForm").submit();

							} else if (data.type == 2) {
								var index = layer
										.confirm(
												'操作人员不对，是否需要更换操作人员？',
												{
													btn : [ '替换,并返回订单',
															'不替换,并返回订单', '取消' ],
													btn3 : function(i, l) {
														closeLayer(index);
														$("#firstInput")
																.val('');
														document
																.getElementById(
																		"firstInput")
																.focus();
													}
												// 按钮
												},
												function() {
													var did = data.detail.id;
													var eid = data.employee.id;
													var url = "worksheetdefatil?goto=workingTemp";
													$
															.post(
																	url,
																	$("#fm1")
																			.serialize(),
																	function() {
																		$(
																				"#eidCom")
																				.val(
																						'');
																		$(
																				"#didCom")
																				.val(
																						'');
																		$("#ot")
																				.val(
																						'');
																		var did = data.detail.id;
																		var eid = data.employee.id;
																		$(
																				"#eidCom")
																				.val(
																						eid);
																		$(
																				"#didCom")
																				.val(
																						did);
																		$(
																				"#workForm")
																				.attr(
																						'action',
																						'worksheet?goto=appView');
																		$
																				.post(
																						'worksheet?goto=updateEMP',
																						{
																							did : did,
																							eid : eid
																						},
																						function(
																								data) {
																							$(
																									"#workForm")
																									.submit();
																						})

																	});
												},
												function() {
													$("#eidCom").val('');
													$("#didCom").val('');
													var did = data.detail.id;
													$("#ot").val('');
													$("#didCom").val(did);
													$("#workForm")
															.attr('action',
																	'worksheet?goto=appView3');
													$("#workForm").submit();

												});
							} else if (data.type == 3) {
								var index = layer
										.open({
											type : 0,
											content : '其他工作台正在操作此订单!',
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ngl5"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ngl5").text(sec);
											$("#firstInput").val('');
											document.getElementById(
													"firstInput").focus();
										});
							} else {
								var msg = data.msg;
								var index = layer
										.open({
											type : 0,
											content : msg,
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ngl6"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ngl6").text(sec);
											$("#firstInput").val('');
											document.getElementById(
													"firstInput").focus();
										});
							}
						}
					});
}

function closeLayer(index) {
	layer.close(index);
	document.getElementById("firstInput").focus();
}

function goPage() {
	window.location.href = "app";
}

function lookMain(wid) {
	$
			.post(
					'worksheet?goto=clickWork',
					{
						wid : wid
					},
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							if (data.type == 1) {
								var wid = data.object.id;
								var url = "worksheetdefatil?goto=workingTemp";
								$.post(url, $("#fm1").serialize(), function() {
									$("#eidCom").val('');
									$("#didCom").val('');
									$("#didCom").val(wid);
									$("#ot").val('');
									$("#workForm").attr('action',
											'worksheet?goto=appView3');
									$("#workForm").submit();
								});
							} else {
								var index = layer
										.open({
											type : 0,
											content : '其他工作台正在操作此订单!',
											area : 'auto',
											title : '无效登录（<span style="color:red;" id="ng_9"></span>秒倒计时）'
										});
								dyncTimer(timer_json["INVAILD_LOGIN"],
										function() {
											closeLayer(index);
										}, function(sec) {
											$("#ng_9").text(sec);
										});
							}
						}
					})
}

function submitForm() {
	var did = $("#didCom").val();
	var eid = $("#eidCom").val();
	if (eid != null && eid != "") {
		$.post('worksheet?goto=updateEMP', {
			did : did,
			eid : eid
		}, function(data) {
			$("#workForm").submit();
		})
	} else {
		$("#workForm").submit();
	}
}

$(function() {
	setInterval(checkIsExist, 1000);
})
var flag = 0;
function checkIsExist() {
	var wid = $("#worksheetIsWorking").val();
	$
			.post(
					"worksheet?goto=isOverTime",
					{
						wid : wid
					},
					function(data) {
						if (typeof (data) == 'string') {
							var index = layer
									.open({
										title : '用户超时'
												+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
										content : '登录用户已超时，请重新登录！',
										btn : '我知道了',
										yes : function(index, layero) {
											window.location.href = "login";
										}
									});
							dyncTimer(timer_json["CONTINUE_SCAN"], function() {
								closeLayer(index);
								window.location.href = "login";
							}, function(sec) {
								$("#overTime_1").text(sec);
							});
						} else {
							if (data.suc == 1) {
								var msg = data.msg;
								if (flag == 0) {
									flag = 1;
									var index = layer
											.open({
												title : msg
														+ '（<span style="color:red;" id="overTime_1"></span>秒倒计时）',
												content : msg,
												btn : '我知道了',
												yes : function(index, layero) {
													window.location.href = "app";
												}
											});
									dyncTimer(timer_json["CONTINUE_SCAN"],
											function() {
												closeLayer(index);
												window.location.href = "app";
											}, function(sec) {
												$("#overTime_1").text(sec);
											});
								}
							}
						}
					});
}

function showPic(id) {
	$("#pic").attr('src', "productElement/showById?id=" + id);
	layer.open({
		type : 1,
		content : $('#pic1'),
		area : 'auto',
		title : '图片样式'
	});
}