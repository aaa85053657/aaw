...
public class CountryCtl extends BaseCtl<Country> {
	...
	public ModelAndView view() {
		return new ModelAndView("sys/country/country");
	}
	...
	@ResponseBody
	public Object save(Country bean, HttpServletRequest request) {
		try {
			service.save(bean);
			return saveSuc(request);
		} catch (Exception e) {
			log.error("CountryCtl保存对象异常", e);
		}
	...
	@ResponseBody
	public Object update(Country bean, HttpServletRequest request) {
		try {
			service.update(bean);
			return updateSuc(request);
		} catch (Exception e) {
			log.error("CountryCtl修改对象异常", e);
		}
	...
	public Object delete(@RequestParam(defaultValue = "0") int id,
		...
		try {
			...
			log.error("CountryCtl通过ID删除对象异常", e);
		}
	...
	public Object unique(String propName, String propVal,
		...
		try {
			...
			log.error("CountryCtl验证是否唯一异常", e);
		}
		return 2;
	}
