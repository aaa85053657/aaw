...
@Controller
@RequestMapping("commandeStatus")
public class CommandeStatusCtl extends BaseCtl<CommandeStatus> {

	@Resource
	private ICommandeStatusService service;
	...
	public Object list(@RequestParam(defaultValue = "1") int page,
		...
		try {
			...
			log.error("CommandeStatusCtl列表查询异常", e);
		}
		return null;
	}
	...
	@ResponseBody
	public Object save(CommandeStatus bean, HttpServletRequest request) {
		try {
			service.save(bean);
			return saveSuc(request);
		} catch (Exception e) {
			log.error("CommandeStatusCtl保存对象异常", e);
		}
	...
	@ResponseBody
	public Object update(CommandeStatus bean, HttpServletRequest request) {
		try {
			service.update(bean);
			return suc("修改成功");
		} catch (Exception e) {
			log.error("CommandeStatusCtl修改对象异常", e);
		}
		return err("修改异常");
	}
	...
	public Object delete(@RequestParam(defaultValue = "0") int id) {
		try {
			...
			log.error("CommandeStatusCtl通过ID删除对象异常", e);
		}
		return err("删除异常");
	}
